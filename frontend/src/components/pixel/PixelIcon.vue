<template>
  <svg
    :width="size"
    :height="size"
    :viewBox="viewBox"
    shape-rendering="crispEdges"
    xmlns="http://www.w3.org/2000/svg"
    :class="['pixel-icon', `pixel-icon--${color}`]"
  >
    <template v-if="name === 'house'">
      <rect x="6" y="2" width="4" height="2" fill="currentColor" />
      <rect x="4" y="4" width="8" height="2" fill="currentColor" />
      <rect x="2" y="6" width="12" height="2" fill="currentColor" />
      <rect x="2" y="8" width="12" height="2" fill="currentColor" />
      <rect x="2" y="10" width="3" height="2" fill="currentColor" />
      <rect x="2" y="12" width="3" height="2" fill="currentColor" />
      <rect x="11" y="10" width="3" height="2" fill="currentColor" />
      <rect x="11" y="12" width="3" height="2" fill="currentColor" />
      <rect x="5" y="10" width="6" height="4" fill="currentColor" />
    </template>

    <template v-else-if="name === 'compass'">
      <rect x="7" y="1" width="2" height="2" fill="currentColor" />
      <rect x="5" y="3" width="6" height="2" fill="currentColor" />
      <rect x="3" y="5" width="2" height="6" fill="currentColor" />
      <rect x="11" y="5" width="2" height="6" fill="currentColor" />
      <rect x="5" y="5" width="6" height="6" fill="currentColor" opacity="0.3" />
      <rect x="7" y="6" width="2" height="4" fill="currentColor" />
      <rect x="6" y="7" width="4" height="2" fill="currentColor" />
      <rect x="3" y="11" width="2" height="2" fill="currentColor" />
      <rect x="11" y="11" width="2" height="2" fill="currentColor" />
      <rect x="5" y="13" width="6" height="2" fill="currentColor" />
      <rect x="7" y="15" width="2" height="1" fill="currentColor" />
    </template>

    <template v-else-if="name === 'chat'">
      <rect x="3" y="3" width="10" height="2" fill="currentColor" />
      <rect x="2" y="5" width="12" height="1" fill="currentColor" />
      <rect x="2" y="6" width="12" height="6" fill="currentColor" opacity="0.3" />
      <rect x="2" y="6" width="12" height="1" fill="currentColor" />
      <rect x="2" y="12" width="8" height="1" fill="currentColor" />
      <rect x="3" y="13" width="6" height="1" fill="currentColor" />
      <rect x="4" y="14" width="4" height="1" fill="currentColor" />
      <rect x="5" y="15" width="2" height="1" fill="currentColor" />
    </template>

    <template v-else-if="name === 'edit'">
      <rect x="11" y="1" width="2" height="2" fill="currentColor" />
      <rect x="12" y="3" width="2" height="2" fill="currentColor" />
      <rect x="9" y="3" width="2" height="2" fill="currentColor" />
      <rect x="7" y="5" width="2" height="2" fill="currentColor" />
      <rect x="5" y="7" width="2" height="2" fill="currentColor" />
      <rect x="3" y="9" width="2" height="2" fill="currentColor" />
      <rect x="2" y="11" width="2" height="2" fill="currentColor" />
      <rect x="2" y="12" width="12" height="2" fill="currentColor" opacity="0.2" />
      <rect x="4" y="13" width="8" height="2" fill="currentColor" />
    </template>

    <template v-else-if="name === 'user'">
      <rect x="6" y="1" width="4" height="2" fill="currentColor" />
      <rect x="4" y="3" width="8" height="2" fill="currentColor" />
      <rect x="3" y="5" width="10" height="4" fill="currentColor" opacity="0.3" />
      <rect x="3" y="5" width="10" height="1" fill="currentColor" />
      <rect x="4" y="9" width="8" height="2" fill="currentColor" />
      <rect x="3" y="11" width="10" height="2" fill="currentColor" />
      <rect x="3" y="13" width="3" height="2" fill="currentColor" />
      <rect x="10" y="13" width="3" height="2" fill="currentColor" />
    </template>

    <template v-else-if="name === 'settings'">
      <rect x="3" y="1" width="2" height="2" fill="currentColor" />
      <rect x="11" y="1" width="2" height="2" fill="currentColor" />
      <rect x="1" y="3" width="2" height="2" fill="currentColor" />
      <rect x="13" y="3" width="2" height="2" fill="currentColor" />
      <rect x="5" y="3" width="6" height="1" fill="currentColor" />
      <rect x="5" y="4" width="6" height="1" fill="currentColor" />
      <rect x="6" y="5" width="4" height="6" fill="currentColor" opacity="0.3" />
      <rect x="5" y="5" width="1" height="6" fill="currentColor" />
      <rect x="10" y="5" width="1" height="6" fill="currentColor" />
      <rect x="6" y="10" width="4" height="1" fill="currentColor" />
      <rect x="2" y="6" width="3" height="4" fill="currentColor" opacity="0.15" />
      <rect x="11" y="6" width="3" height="4" fill="currentColor" opacity="0.15" />
      <rect x="2" y="11" width="12" height="2" fill="currentColor" />
      <rect x="4" y="13" width="8" height="2" fill="currentColor" />
    </template>

    <template v-else-if="name === 'star'">
      <rect x="8" y="1" width="1" height="2" fill="currentColor" />
      <rect x="6" y="3" width="5" height="2" fill="currentColor" />
      <rect x="3" y="5" width="11" height="2" fill="currentColor" />
      <rect x="2" y="7" width="13" height="2" fill="currentColor" />
      <rect x="4" y="9" width="9" height="2" fill="currentColor" />
      <rect x="5" y="11" width="3" height="2" fill="currentColor" />
      <rect x="9" y="11" width="3" height="2" fill="currentColor" />
      <rect x="6" y="13" width="2" height="2" fill="currentColor" />
      <rect x="9" y="13" width="2" height="2" fill="currentColor" />
    </template>

    <template v-else-if="name === 'tag'">
      <rect x="1" y="4" width="2" height="8" fill="currentColor" />
      <rect x="3" y="2" width="10" height="2" fill="currentColor" />
      <rect x="3" y="4" width="12" height="8" fill="currentColor" opacity="0.2" />
      <rect x="3" y="4" width="12" height="1" fill="currentColor" />
      <rect x="3" y="11" width="12" height="1" fill="currentColor" />
      <rect x="13" y="2" width="3" height="6" fill="currentColor" opacity="0.5" />
      <rect x="13" y="8" width="2" height="4" fill="currentColor" opacity="0.3" />
      <rect x="6" y="6" width="4" height="4" fill="currentColor" />
      <rect x="6" y="6" width="1" height="4" fill="currentColor" opacity="0.5" />
    </template>

    <template v-else-if="name === 'folder'">
      <rect x="2" y="3" width="5" height="2" fill="currentColor" />
      <rect x="2" y="5" width="13" height="2" fill="currentColor" />
      <rect x="2" y="7" width="13" height="1" fill="currentColor" />
      <rect x="2" y="8" width="13" height="6" fill="currentColor" opacity="0.2" />
      <rect x="2" y="8" width="13" height="6" fill="currentColor" opacity="0.4" />
      <rect x="2" y="8" width="13" height="1" fill="currentColor" />
    </template>

    <template v-else-if="name === 'bell'">
      <rect x="6" y="1" width="4" height="2" fill="currentColor" />
      <rect x="4" y="3" width="8" height="2" fill="currentColor" />
      <rect x="3" y="5" width="10" height="2" fill="currentColor" />
      <rect x="3" y="7" width="10" height="4" fill="currentColor" opacity="0.2" />
      <rect x="3" y="7" width="10" height="1" fill="currentColor" />
      <rect x="5" y="11" width="6" height="2" fill="currentColor" />
      <rect x="6" y="13" width="4" height="2" fill="currentColor" />
      <rect x="12" y="8" width="2" height="1" fill="currentColor" />
    </template>

    <template v-else-if="name === 'heart'">
      <rect x="4" y="3" width="3" height="2" fill="currentColor" />
      <rect x="9" y="3" width="3" height="2" fill="currentColor" />
      <rect x="2" y="5" width="12" height="2" fill="currentColor" />
      <rect x="1" y="7" width="14" height="2" fill="currentColor" />
      <rect x="1" y="9" width="14" height="2" fill="currentColor" />
      <rect x="2" y="11" width="12" height="2" fill="currentColor" />
      <rect x="3" y="13" width="10" height="2" fill="currentColor" />
      <rect x="5" y="15" width="6" height="2" fill="currentColor" />
      <rect x="7" y="17" width="2" height="1" fill="currentColor" />
    </template>

    <template v-else-if="name === 'search'">
      <rect x="3" y="2" width="8" height="8" fill="currentColor" opacity="0.2" />
      <rect x="2" y="1" width="10" height="1" fill="currentColor" />
      <rect x="2" y="9" width="10" height="1" fill="currentColor" />
      <rect x="1" y="2" width="1" height="7" fill="currentColor" />
      <rect x="11" y="2" width="1" height="7" fill="currentColor" />
      <rect x="10" y="10" width="2" height="2" fill="currentColor" />
      <rect x="12" y="12" width="2" height="2" fill="currentColor" />
      <rect x="6" y="4" width="2" height="2" fill="currentColor" />
      <rect x="6" y="6" width="1" height="1" fill="currentColor" />
    </template>

    <template v-else-if="name === 'arrow-right'">
      <rect x="4" y="6" width="3" height="1" fill="currentColor" />
      <rect x="6" y="5" width="8" height="3" fill="currentColor" opacity="0.4" />
      <rect x="7" y="4" width="4" height="1" fill="currentColor" />
      <rect x="8" y="3" width="3" height="1" fill="currentColor" />
      <rect x="7" y="9" width="4" height="1" fill="currentColor" />
      <rect x="8" y="10" width="3" height="1" fill="currentColor" />
    </template>

    <template v-else-if="name === 'arrow-down'">
      <rect x="6" y="4" width="1" height="3" fill="currentColor" />
      <rect x="4" y="7" width="5" height="1" fill="currentColor" />
      <rect x="5" y="8" width="3" height="1" fill="currentColor" />
      <rect x="6" y="9" width="1" height="1" fill="currentColor" />
    </template>

    <template v-else-if="name === 'plus'">
      <rect x="6" y="1" width="4" height="14" fill="currentColor" />
      <rect x="1" y="6" width="14" height="4" fill="currentColor" />
    </template>

    <template v-else-if="name === 'menu'">
      <rect x="1" y="3" width="14" height="2" fill="currentColor" />
      <rect x="1" y="7" width="14" height="2" fill="currentColor" />
      <rect x="1" y="11" width="14" height="2" fill="currentColor" />
    </template>

    <template v-else-if="name === 'close'">
      <rect x="2" y="1" width="3" height="2" fill="currentColor" />
      <rect x="11" y="1" width="3" height="2" fill="currentColor" />
      <rect x="4" y="3" width="2" height="2" fill="currentColor" />
      <rect x="10" y="3" width="2" height="2" fill="currentColor" />
      <rect x="6" y="5" width="4" height="6" fill="currentColor" />
      <rect x="4" y="11" width="2" height="2" fill="currentColor" />
      <rect x="10" y="11" width="2" height="2" fill="currentColor" />
      <rect x="2" y="13" width="3" height="2" fill="currentColor" />
      <rect x="11" y="13" width="3" height="2" fill="currentColor" />
    </template>

    <template v-else-if="name === 'logout'">
      <rect x="1" y="3" width="9" height="2" fill="currentColor" />
      <rect x="1" y="5" width="9" height="6" fill="currentColor" opacity="0.2" />
      <rect x="1" y="11" width="9" height="2" fill="currentColor" />
      <rect x="10" y="7" width="4" height="1" fill="currentColor" />
      <rect x="12" y="5" width="2" height="1" fill="currentColor" />
      <rect x="12" y="9" width="2" height="1" fill="currentColor" />
      <rect x="13" y="6" width="1" height="3" fill="currentColor" />
    </template>

    <template v-else-if="name === 'document'">
      <rect x="3" y="1" width="9" height="2" fill="currentColor" />
      <rect x="2" y="3" width="11" height="1" fill="currentColor" />
      <rect x="2" y="4" width="11" height="10" fill="currentColor" opacity="0.15" />
      <rect x="2" y="4" width="11" height="1" fill="currentColor" />
      <rect x="2" y="11" width="11" height="1" fill="currentColor" />
      <rect x="4" y="6" width="7" height="1" fill="currentColor" opacity="0.5" />
      <rect x="4" y="8" width="5" height="1" fill="currentColor" opacity="0.5" />
      <rect x="6" y="13" width="2" height="2" fill="currentColor" />
    </template>

    <template v-else-if="name === 'lock'">
      <rect x="5" y="1" width="6" height="2" fill="currentColor" />
      <rect x="4" y="3" width="8" height="2" fill="currentColor" />
      <rect x="3" y="5" width="10" height="2" fill="currentColor" />
      <rect x="3" y="7" width="10" height="8" fill="currentColor" opacity="0.2" />
      <rect x="3" y="7" width="10" height="1" fill="currentColor" />
      <rect x="6" y="9" width="4" height="4" fill="currentColor" />
      <rect x="7" y="11" width="3" height="1" fill="var(--pixel-bg-base)" />
    </template>

    <template v-else-if="name === 'clock'">
      <rect x="7" y="1" width="2" height="2" fill="currentColor" />
      <rect x="4" y="3" width="8" height="2" fill="currentColor" />
      <rect x="2" y="5" width="12" height="2" fill="currentColor" />
      <rect x="1" y="7" width="14" height="4" fill="currentColor" opacity="0.15" />
      <rect x="1" y="7" width="1" height="4" fill="currentColor" />
      <rect x="14" y="7" width="1" height="4" fill="currentColor" />
      <rect x="2" y="11" width="12" height="2" fill="currentColor" />
      <rect x="4" y="13" width="8" height="2" fill="currentColor" />
      <rect x="7" y="15" width="2" height="1" fill="currentColor" />
      <rect x="8" y="6" width="1" height="4" fill="currentColor" />
      <rect x="8" y="6" width="3" height="1" fill="currentColor" />
    </template>

    <template v-else-if="name === 'article'">
      <rect x="2" y="2" width="12" height="2" fill="currentColor" />
      <rect x="2" y="5" width="12" height="8" fill="currentColor" opacity="0.15" />
      <rect x="2" y="5" width="12" height="1" fill="currentColor" opacity="0.3" />
      <rect x="2" y="9" width="12" height="1" fill="currentColor" opacity="0.3" />
      <rect x="4" y="6" width="8" height="1" fill="currentColor" opacity="0.4" />
      <rect x="4" y="7" width="6" height="1" fill="currentColor" opacity="0.4" />
      <rect x="3" y="14" width="3" height="1" fill="currentColor" />
      <rect x="7" y="14" width="3" height="1" fill="currentColor" />
      <rect x="11" y="14" width="3" height="1" fill="currentColor" />
    </template>

    <template v-else-if="name === 'dashboard'">
      <rect x="1" y="1" width="6" height="5" fill="currentColor" opacity="0.2" />
      <rect x="1" y="1" width="6" height="1" fill="currentColor" />
      <rect x="9" y="1" width="6" height="5" fill="currentColor" opacity="0.2" />
      <rect x="9" y="1" width="6" height="1" fill="currentColor" />
      <rect x="1" y="8" width="6" height="5" fill="currentColor" opacity="0.2" />
      <rect x="1" y="8" width="6" height="1" fill="currentColor" />
      <rect x="9" y="8" width="6" height="5" fill="currentColor" opacity="0.2" />
      <rect x="9" y="8" width="6" height="1" fill="currentColor" />
    </template>

    <template v-else-if="name === 'pixel-book'">
      <rect x="2" y="2" width="4" height="12" fill="currentColor" />
      <rect x="6" y="2" width="8" height="2" fill="currentColor" />
      <rect x="6" y="4" width="8" height="1" fill="currentColor" />
      <rect x="6" y="5" width="8" height="7" fill="currentColor" opacity="0.2" />
      <rect x="6" y="5" width="8" height="1" fill="currentColor" opacity="0.4" />
      <rect x="6" y="12" width="8" height="2" fill="currentColor" />
    </template>

    <template v-else-if="name === 'pixel-sparkle'">
      <rect x="8" y="1" width="2" height="2" fill="currentColor" />
      <rect x="3" y="3" width="2" height="2" fill="currentColor" />
      <rect x="13" y="3" width="2" height="2" fill="currentColor" />
      <rect x="8" y="7" width="2" height="2" fill="currentColor" />
      <rect x="6" y="8" width="6" height="1" fill="currentColor" />
      <rect x="2" y="11" width="2" height="2" fill="currentColor" />
      <rect x="14" y="11" width="2" height="2" fill="currentColor" />
    </template>

    <template v-else-if="name === 'pixel-shield'">
      <rect x="6" y="1" width="4" height="2" fill="currentColor" />
      <rect x="4" y="3" width="8" height="2" fill="currentColor" />
      <rect x="3" y="5" width="10" height="2" fill="currentColor" />
      <rect x="2" y="7" width="12" height="2" fill="currentColor" />
      <rect x="3" y="9" width="10" height="2" fill="currentColor" />
      <rect x="4" y="11" width="8" height="2" fill="currentColor" />
      <rect x="5" y="13" width="6" height="2" fill="currentColor" />
      <rect x="6" y="15" width="4" height="1" fill="currentColor" />
      <rect x="6" y="6" width="4" height="1" fill="currentColor" />
      <rect x="8" y="2" width="1" height="3" fill="currentColor" />
    </template>

    <template v-else-if="name === 'pixel-coin'">
      <rect x="5" y="2" width="7" height="2" fill="currentColor" />
      <rect x="3" y="4" width="11" height="2" fill="currentColor" />
      <rect x="2" y="6" width="13" height="6" fill="currentColor" opacity="0.25" />
      <rect x="2" y="6" width="13" height="1" fill="currentColor" />
      <rect x="2" y="11" width="13" height="1" fill="currentColor" />
      <rect x="3" y="12" width="11" height="2" fill="currentColor" />
      <rect x="5" y="14" width="7" height="2" fill="currentColor" />
      <rect x="7" y="8" width="3" height="3" fill="currentColor" />
    </template>
  </svg>
</template>

<script setup lang="ts">
withDefaults(defineProps<{
  name: 'house' | 'compass' | 'chat' | 'edit' | 'user' | 'settings' | 'star'
    | 'tag' | 'folder' | 'bell' | 'heart' | 'search' | 'arrow-right'
    | 'arrow-down' | 'plus' | 'menu' | 'close' | 'logout' | 'document'
    | 'lock' | 'clock' | 'article' | 'dashboard' | 'pixel-book'
    | 'pixel-sparkle' | 'pixel-shield' | 'pixel-coin'
  size?: number | string
  color?: 'mint' | 'pink' | 'gold' | 'purple' | 'white'
}>(), {
  size: 24,
  color: 'mint',
})

const viewBox = '0 0 16 16'
</script>

<style scoped>
.pixel-icon {
  display: inline-block;
  flex-shrink: 0;
  vertical-align: middle;
}

.pixel-icon--mint { color: var(--pixel-accent-mint); }
.pixel-icon--pink { color: var(--pixel-accent-pink); }
.pixel-icon--gold { color: var(--pixel-accent-gold); }
.pixel-icon--purple { color: var(--pixel-accent-purple); }
.pixel-icon--white { color: var(--pixel-text-primary); }
</style>
